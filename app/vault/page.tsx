...
function VaultContent() {
  ...
  // ✅ Pre-fill password from generator context (secure)
  useEffect(() => {
    if (generatedPassword && mounted && vaultAccessVerified) {
      setFormData(prev => ({ ...prev, password: generatedPassword }));
      setShowModal(true);
      // Clear the password from context after using it
  ...
  const handleSetKey = async () => {
    ...
    setKeyError(""); // Clear previous errors
    setKeyLoading(true);
    
    // Validate the key by trying to decrypt existing items
    try {
      ...
      if (data.success && data.items.length > 0) {
        // Try to decrypt the first item to validate the key
      ...
      // Key is valid (or no items to validate against)
  ...
  // Show loading while access verification is in progress
  if (!vaultAccessVerified) {
    return (
      <>
        ...
        {/* Vault Access Verification Modal */}
        <VaultAccessModal
